...
Source: [${testHelper_1.SCHEMA_ID}](file:///${testHelper_1.SCHEMA_ID})`);
        ...
        it('Hover displays escaped quote strings correctly in enum', async () => {
            ...
            assert.strictEqual(vscode_languageserver_types_1.MarkupContent.is(result.contents), true);
            assert.strictEqual(result.contents.kind, 'markdown');
            assert.strictEqual(result.contents.value, `Test enum with special quote strings
...
Source: [${testHelper_1.SCHEMA_ID}](file:///${testHelper_1.SCHEMA_ID})`);
        ...
        it('Hover preserves literal parentheses', async () => {
            ...
            assert.strictEqual(vscode_languageserver_types_1.MarkupContent.is(hover.contents), true);
            assert.strictEqual(hover.contents.kind, 'markdown');
            assert.strictEqual(hover.contents.value, `Parentheses should be literal: (abc)(1).\n\nSource: [${testHelper_1.SCHEMA_ID}](file:///${testHelper_1.SCHEMA_ID})`);
        });
        it('Hover preserves literal dots', async () => {
            ...
            assert.strictEqual(vscode_languageserver_types_1.MarkupContent.is(hover.contents), true);
            assert.strictEqual(hover.contents.kind, 'markdown');
            assert.strictEqual(hover.contents.value, `Dots should be literal in normal text: v1.2.3, 3.14159, example.com.\n\nSource: [${testHelper_1.SCHEMA_ID}](file:///${testHelper_1.SCHEMA_ID})`);
        });
        it('Hover preserves bare URL in description (no escaping)', async () => {
            ...
            assert.strictEqual(vscode_languageserver_types_1.MarkupContent.is(hover.contents), true);
            assert.strictEqual(hover.contents.kind, 'markdown');
            assert.strictEqual(hover.contents.value, `The YAML Language Server at https://github.com/redhat\\-developer/yaml\\-language\\-server\n\nSource: [${testHelper_1.SCHEMA_ID}](file:///${testHelper_1.SCHEMA_ID})`);
        });
        it('Hover preserves URL wrapped in parentheses in description (no escaping)', async () => {
            ...
            assert.strictEqual(vscode_languageserver_types_1.MarkupContent.is(hover.contents), true);
            assert.strictEqual(hover.contents.kind, 'markdown');
            assert.strictEqual(hover.contents.value, `Yaml Language Server (https://github.com/redhat\\-developer/yaml\\-language\\-server)\n\nSource: [${testHelper_1.SCHEMA_ID}](file:///${testHelper_1.SCHEMA_ID})`);
        });
    });
    ...
    describe('YAML 1.1 boolean hover', () => {
        let yaml11LanguageHandler;
        let yaml11Settings;
        let yaml11SchemaProvider;
        before(() => {
            const yaml11Setup = new serviceSetup_1.ServiceSetup().withHover().withYamlVersion('1.1');
            const { languageHandler: langHandler, yamlSettings: settings, schemaProvider: testSchemaProvider, } = (0, testHelper_1.setupLanguageService)(yaml11Setup.languageSettings);
            yaml11LanguageHandler = langHandler;
            yaml11Settings = settings;
            yaml11SchemaProvider = testSchemaProvider;
        });
        afterEach(() => {
            yaml11SchemaProvider.deleteSchema(testHelper_1.SCHEMA_ID);
        });
        function parseSetupYaml11(content, position) {
            if (typeof position === 'undefined') {
                ({ content, position } = (0, testHelper_1.caretPosition)(content));
            }
            const testTextDocument = (0, testHelper_1.setupSchemaIDTextDocument)(content);
            yaml11Settings.documents = new yamlSettings_1.TextDocumentTestManager();
            yaml11Settings.documents.set(testTextDocument);
        ...
        it('Hover on YAML 1.1 boolean value "True" with enum schema', async () => {
            yaml11SchemaProvider.addSchema(testHelper_1.SCHEMA_ID, {
                type: 'object',
                properties: {
                    enabled: {
                        type: 'boolean',
            ...
            assert.strictEqual(vscode_languageserver_types_1.MarkupContent.is(result.contents), true);
            assert.notStrictEqual(result.contents.value, '');
            (0, chai_1.expect)(result.contents.value).to.include('Enable or disable the feature');
        });
        ...
        it('Hover on YAML 1.1 boolean value "yes" with const schema', async () => {
            ...
            assert.strictEqual(vscode_languageserver_types_1.MarkupContent.is(result.contents), true);
            assert.notStrictEqual(result.contents.value, '');
            (0, chai_1.expect)(result.contents.value).to.include('Confirmation flag');
        });
        it('Hover on YAML 1.1 boolean value "no" with const schema', async () => {
            ...
            assert.strictEqual(vscode_languageserver_types_1.MarkupContent.is(result.contents), true);
            assert.notStrictEqual(result.contents.value, '');
            (0, chai_1.expect)(result.contents.value).to.include('Disabled flag');
        });
    });
});
