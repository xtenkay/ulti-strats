...
async function createTypeScriptServerProject(ts, tsLocalized, tsconfig, server, serviceEnv, getLanguagePlugins) {
    ...
    let languageService;
    const sys = (0, typescript_1.createSys)(ts, serviceEnv, (0, uri_1.uriToFileName)(serviceEnv.workspaceFolder));
    const host = {
        ...sys,
        ...
        getCurrentDirectory() {
            return (0, uri_1.uriToFileName)(serviceEnv.workspaceFolder);
        },
        getProjectVersion() {
    ...
    const askedFiles = (0, uriMap_1.createUriMap)(uri_1.fileNameToUri);
    ...
    async function getRootFiles(languagePlugins) {
        parsedCommandLine = await createParsedCommandLine(ts, sys, (0, uri_1.uriToFileName)(serviceEnv.workspaceFolder), tsconfig, languagePlugins.map(plugin => plugin.typescript?.extraFileExtensions ?? []).flat());
