...
function register(context) {
    ...
    return async (uri, options, range, onTypeParams, token = cancellation_1.NoneCancellationToken) => {
        ...
        try {
            ...
            let level = 0;
            while (true) {
                const embeddedCodes = (0, featureWorkers_1.getEmbeddedFilesByLevel)(context, sourceScript.id, tempVirtualFile, level++);
                ...
                for (const code of embeddedCodes) {
                    ...
                    if (onTypeParams) {
                        ...
                        if (embeddedPosition) {
                            embeddedCodeResult = await tryFormat(docMap.sourceDocument, docMap.embeddedDocument, sourceScript, code, level, embeddedPosition, onTypeParams.ch);
                        }
                    }
                    else if (embeddedRange) {
                        embeddedCodeResult = await tryFormat(docMap.sourceDocument, docMap.embeddedDocument, sourceScript, code, level, {
